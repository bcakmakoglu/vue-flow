...
Split the store API to match `useReactFlow`/`useSvelteFlow`. `useVueFlow()` is now the curated **instance** — actions, computed getters, and event hooks (`onNodeClick`, `onConnect`, …) — and no longer exposes the raw reactive state. The state moved to a new `useStore()`, with `storeToRefs()` as the destructure bridge (Pinia-style).
