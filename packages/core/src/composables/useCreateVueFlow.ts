...
/** Generate a unique flow id (for aria/debug — not a lookup key). */
 ...
 * `provide` + `watch`, which bind to the component scope).
 *
 * @internal
 */
export function useCreateVueFlow<NodeType extends Node = Node>(
  ...
  const store = createVueFlowStore<NodeType>(options?.id ?? generateFlowId(), options, undefined, signals)
